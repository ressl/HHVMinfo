...
function print_table($array,$headers=false, $formatkeys=false, $formatvalues=false ) {
	if ( empty($array) || !is_array($array) ) {return;} 
  	...
  	foreach ($array as $key=>$value) {
    		...
    		if ( !is_numeric($key) || !$formatkeys ) { 
      			if ($formatkeys) { $key=ucwords(str_replace('_',' ',$key)); }
      			echo '<td class="e">',$key,'</td>'; 
      			if ( $formatvalues && is_numeric($value) ) {
